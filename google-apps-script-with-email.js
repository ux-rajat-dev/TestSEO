...
function sendQuoteEmail(data) {
  try {
    ...
    if (!data || !data.email || !data.name) {
      console.log('Missing required data for email:', data);
      return;
    }
    ...
    let cartItems = [];
    try {
      if (data.cart_items && data.cart_items !== '') {
    ...
    if (cartItems && cartItems.length > 0) {
      cartItemsText = cartItems.map(item => {
        const price = item.price || 0;
        const period = item.period || 'month';
        return `${item.name} (€${price}/${period})`;
      }).join('\n');
    } else {
      cartItemsText = data.selected_plan || 'No items selected';
    }
    
    // Calculate VAT (21%) safely
    ...
    // Create email body with safe property access
    const emailBody = `
...
function testEmail() {
  const testData = {
    name: 'Paladugu Ankitha',
    company: 'Test Company',
    email: 'test@example.com', // Change this to your email for testing
    selected_plan: 'eBranch Plan',
    cart_items: JSON.stringify([{name: 'eBranch Plan', price: 1995, period: 'year'}]),
    total_amount: 1995,
    timestamp: new Date().toISOString()
  };
